...
export default function HomeScreen({navigation}) {
  ...
  return (
      ...
      <View style={{backgroundColor:`${theme}`}}>
        ...
        value={isEnabled}/>
        <Text style={{fontSize:40,margin:60,marginLeft:80}}>Find Your Latest Gadgets</Text> 
        <Image source={require('./assets/image.png')} style={style2.image}/>
      <Pressable style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'flex-end'}}>
        <Text style={[style2.button,{color:"#7c42da",backgroundColor:`${color}`}]} onPress={()=>navigation.navigate('SignUp')}>Get Started With The App</Text>
      </Pressable>
      </View>
  </SafeAreaView>
